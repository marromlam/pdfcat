...
class NoteManager:
    ...
    def _resolve_notes_dir(doc: Any) -> tuple[str | None, str | None]:
        ...
        if cfg is not None and "NOTE_PATH" in loaded_keys and "NOTES_DIR" not in loaded_keys:
    ...
    def resolve_note_path(doc: Any) -> tuple[str | None, str | None]:
        ...
        title = resolve_title() if callable(resolve_title) else NoteManager._note_title(doc)
        ...
        try:
            if ".." in note_filename or "/" in note_filename or "\\" in note_filename:
                logging.error("Note filename contains path separators: %s", note_filename)
    ...
    def open_notes_editor(doc: Any) -> str | None:
        ...
        try:
            ...
            if in_tmux and tmux_bin is not None:
                ...
                proc = subprocess.run([tmux_bin, "display-popup", "-E", cmd], check=False)
            else:
