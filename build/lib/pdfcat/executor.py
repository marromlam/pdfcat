...
class ActionExecutor:
    ...
    def execute(self, action: Action, doc: Any, bar: Any) -> Any:
        ...
        if isinstance(action, BufferSwitchAction):
            self.buffers.switch_to_buffer(action.index)
            doc = self.buffers.docs[self.buffers.current]
            return self._activate_doc(doc, bar)

        if isinstance(action, BufferCloseAction):
            self.buffers.close_buffer(self.buffers.current)
            doc = self.buffers.docs[self.buffers.current]
            return self._activate_doc(doc, bar)
