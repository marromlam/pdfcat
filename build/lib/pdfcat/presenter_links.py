...
class PresenterLinks:
    ...
    def _get_actionable_links(self) -> list[dict[str, Any]]:
        ...
        return [link for link in links if link.get("kind", 0) > 0 and link.get("from") is not None]
    ...
    def _get_hintable_links(self) -> list[dict[str, Any]]:
        ...
        if self.doc.manualcrop and self.doc.manualcroprect != [None, None] and self.doc.is_pdf:
            page.set_cropbox(fitz.Rect(self.doc.manualcroprect[0], self.doc.manualcroprect[1]))
    ...
    def show_link_hints(self, bar: Any) -> None:
        ...
        if not (is_kitty_proto or is_supported_name):
            bar.message = "Link hints require kitty/tmux+kitty renderer; use F for full list"
            return
    ...
    def _format_link_target(self, link: dict[str, Any]) -> str:
        ...
        if kind == 5:
            return f"external {link.get('fileSpec', '')}#{link.get('page', '?')}".strip()
        if kind == 4:
            return str(
                link.get("name")
                or link.get("nameddest")
    ...
    def _show_links_fzf(self, links: list[dict[str, Any]], bar: Any) -> bool:
        ...
        for i, link in enumerate(links):
            ...
            kind = int(link.get("kind", 0))
            kind_label = {1: "INT", 2: "URL", 3: "LAUNCH", 5: "EXTPDF"}.get(kind, f"K{kind}")
