...
class KittyRenderer(RenderingEngine):
    ...
    def detect_support(self) -> bool:
        ...
        try:
            return self._write_gr_cmd_with_response(
                dict(a="q", s=1, v=1, i=1), standard_b64encode(b"abcd")
            )
        except Exception:
            return False

    def _serialize_gr_command(self, cmd: dict[str, Any], payload: bytes | None = None) -> bytes:
    ...
    def _write_gr_cmd_with_response(
        ...
        self._write_gr_cmd(cmd, payload)
        resp = b""
        try:
            while resp[-2:] != b"\033\\":
                if shutdown_event is not None and shutdown_event.is_set():
    ...
    def render_pixmap(
        ...
        success = self._write_gr_cmd_with_response(cmd)
        return success
...
class NativeRenderer(RenderingEngine):
    ...
    def _detect_graphics_protocol(self) -> str:
        ...
        kitty_env = bool(os.environ.get("KITTY_WINDOW_ID") or os.environ.get("KITTY_PID"))
    ...
    def _serialize_gr_command(self, cmd: dict[str, Any], payload: bytes | None = None) -> bytes:
        ...
        if self.in_tmux and self.protocol == "kitty-tmux":
            ...
            data = self._TMUX_START + data.replace(b"\033", b"\033\033") + self._TMUX_END
        return data
    ...
    def _upload_png(
        self, image_id: int, png_data: bytes, width_cells: int, height_cells: int
    ) -> None:
    ...
    def _maybe_clear_tmux_history(self, force: bool = False) -> None:
        if not self.in_tmux:
            return
        now = monotonic()
        if not force and (now - self._last_tmux_history_clear) < self._tmux_history_clear_interval:
            return
        self._last_tmux_history_clear = now
        try:
            subprocess.run(["tmux", "clear-history"], check=False, stderr=subprocess.DEVNULL)
        except Exception:
            pass
    ...
    def render_pixmap(
        ...
        if b_row >= screen.rows:
            height_cells = max(1, max_bottom_row - t_row)
        ...
        try:
            self._upload_png(image_id, png_data, width_cells, height_cells)
            if self.in_tmux and self.protocol == "kitty-tmux":
                self._emit_tmux_placeholders(image_id, indent_cols, height_cells, width_cells)
            else:
                self._write_gr_cmd(
                    {
                        "a": "p",
