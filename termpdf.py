...
class Screen:
    ...
    def swallow_keys(self):
        ...
        k = self.stdscr.getch()  # noqa  # type: ignore
        end = monotonic() + 0.1
        while monotonic() < end:
...
class Document(fitz.Document):
    ...
    def next_page(self, count: int = 1):
        _page = int(self.page) if self.page else 0  # type: ignore
        self.goto_page(_page + count)

    def prev_page(self, count: int = 1):
        _page = int(self.page) if self.page else 0  # type: ignore
        self.goto_page(_page - count)
...
if __name__ == "__main__":
    log_file = os.path.join(os.path.expanduser("~"), ".termpdf.log")
