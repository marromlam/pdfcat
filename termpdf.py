...
class Screen:
    ...
    def create_text_win(self, length, header):
        ...
        self.stdscr.clear()  # type: ignore
        self.stdscr.refresh()  # type: ignore
        win.refresh()
    ...
    def swallow_keys(self):
        self.stdscr.nodelay(True)  # type: ignore
        # k = self.stdscr.getch()  # type: ignore
        end = monotonic() + 0.1
        while monotonic() < end:
            self.stdscr.getch()  # type: ignore
        self.stdscr.nodelay(False)  # type: ignore
...
def get_cachefile(path):
    filehash = path.replace("/", "_").replace(".", "_").replace(" ", "_")
    cachedir = os.path.expanduser(
        os.path.join(os.getenv("XDG_CACHE_HOME", "~/.cache"), "termpdf.py")
    )
...
class Document(fitz.Document):
    ...
    def __init__(
        ...
        self.logical_pages = list(
            range(
                0 + self.first_page_offset,
                self.pages + self.first_page_offset,
            )
        )
        self.chapter = 0
        self.rotation = 0
    ...
    def set_pagelabel(self, count, style="arabic"):
        if self.is_pdf:
            ...
            newlabel = PageLabelScheme(
                startpage=self.page, style=style, prefix="", firstpagenum=count
            )
    ...
    def make_link(self):
        ...
        if self.citekey:
            ...
            return "({}, {}, {})".format(
                self.metadata["author"], self.metadata["title"], p
            )
    ...
    def init_neovim_bridge(self):
        ...
        try:
            ...
            ncmd = "env NVIM_LISTEN_ADDRESS={} nvim {}".format(
                self.nvim_listen_address, config.NOTE_PATH
            )
            try:
...
def detect_support():
    return write_gr_cmd_with_response(
        dict(a="q", s=1, v=1, i=1), standard_b64encode(b"abcd")
    )
...
def bib_from_field(field, regex):
    if shutil.which("bibtool") is not None:
        ...
        select = select + "}"
        text = subprocess.run(
            ["bibtool", "-r", "biblatex", "--", select, config.BIBTEX],
...
def parse_args(args):
    ...
    for i, arg in enumerate(args):
        ...
        elif arg in {"--invert-colors"}:
            opts["invert"] = True
        elif arg in {"--transparent"}:
            opts["alpha"] = True
...
def view(file_change, doc):
    ...
    if not detect_support():
        raise SystemExit("Terminal does not support kitty graphics protocol")
    scr.swallow_keys()
    ...
    while True:
        ...
        scr.stdscr.nodelay(True)  # type: ignore
        key = scr.stdscr.getch()  # type: ignore
        while key == -1 and not file_change.is_set():
            key = scr.stdscr.getch()  # type: ignore
        scr.stdscr.nodelay(False)  # type: ignore
...
def main(args=sys.argv):
    ...
    file_watch = threading.Thread(
        target=watch_for_file_change, args=(file_change, doc.filename)
    )
...
if __name__ == "__main__":
    log_file = os.path.expanduser("~/.termpdf.log")
