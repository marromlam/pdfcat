...
def run_reverse_synctex_to_neovim(doc: Document, bar: status_bar) -> None:
    ...
    if not os.path.isabs(input_path):
        input_path = os.path.abspath(
            os.path.join(os.path.dirname(doc.filename), input_path)
        )
...
def _presenter_enable(doc: Document) -> str:
    ...
    subprocess.run(
        [tmux_bin, "set-option", "-t", session, "status", "off"], check=False
    )
    ...
    for page_num in pages_to_prerender:
        ...
        try:
            ...
            if doc.manualcrop and doc.manualcroprect != [None, None] and doc.is_pdf:
                page.set_cropbox(
                    fitz.Rect(doc.manualcroprect[0], doc.manualcroprect[1])
                )
...
def main(args: Sequence[str] | None = None) -> None:
    ...
    for path in paths:
        try:
            ...
            raise SystemExit(f"Unable to open {path}: {exc}") from exc
        ...
        if os.path.exists(cachefile) and not opts["ignore_cache"]:
            ...
            apply_cached_state(
                ...
                cached_state,
                ignore_visual_state=bool(
                    opts.get("force_tinted") or opts.get("force_original")
                ),
            )
        buffers.docs += [doc]
