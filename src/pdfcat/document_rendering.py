...
def _append_cols_debug_line(doc: Any, payload: dict[str, Any]) -> None:
    """Append non-tmux native placement diagnostics to cols.txt.

    This is intentionally best-effort and must never break rendering.
    """
    try:
        renderer = doc._renderer()
        if renderer is None:
            return
        if getattr(renderer, "name", "") != "native":
            return
        if bool(getattr(renderer, "in_tmux", False)):
            return
        if str(getattr(renderer, "protocol", "")) != "kitty":
            return
        ...
        # Never fail rendering because debug logging failed.
        return
...
def display_page(doc: Any, bar, p, display: bool = True) -> None:
    ...
    # Terminal cursor rows are 1-based; keep placement in the same coordinate space.
    t_row = ((max_rows - page_rows) // 2) + 1
    ...
    _append_cols_debug_line(
        doc,
        {
            ...
            "cell_w_px": screen.cell_width,
            "cell_h_px": screen.cell_height,
            "status_rows": status_rows,
            "page_w_pt": round(pw, 3),
            "page_h_pt": round(ph, 3),
            "zoom_factor": round(factor, 6),
            "zoom_w_px": round(zw, 3),
            "zoom_h_px": round(zh, 3),
            "page_cols": page_cols,
            "page_rows": page_rows,
            "l_col": l_col,
            "t_row": t_row,
            "r_col": r_col,
            "b_row": b_row,
            "expected_left": ((max_cols - page_cols) // 2) + 1,
            "expected_top": ((max_rows - page_rows) // 2) + 1,
        },
    )
    ...
    if page_state.stale or display:
        ...
        visual_cache_hit = (
            cache_hit and cached_ppm is not None and cached_visual_key == visual_key
        )
        ...
        if not visual_cache_hit:
            ...
            page_state.set_cached_ppm(
                None
            )  # Regenerate encoded payload for this visual mode.
